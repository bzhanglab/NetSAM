...
result <- NetSAM(inputNetwork, outputFileName, NetGestalt=TRUE, minModule=(-1), maxStep=4, 
...
This section describes the arguments of the NetSAM function:
	...
	If parameter \emph{NetGestalt} is TRUE, the output file will be used as NetGestalt input. Because current version NetGestalt only support official Human HUGO gene symbols, the node names of the input network must be official Human HUGO gene symbols. If only less than 20 percentage of the node names in the input file are not official HUGO gene symbols, the function will remove the edges involving these node names. If more than 20 percentage of the node names in the input file are not official HUGO gene symbols, the function will consider the input file invalid and the user should convert the node names to official HUGO gene symbols. If parameter \emph{NetGestalt} is FALSE, the output file will be used as other network analysis. Thus, the users can use any type of identifiers as node names of the input network.
...
\subsection{Output}
When \emph{NetGestalt} is TRUE, the NetSAM function outputs a "nsm" file that can be used as an input for the one-dimensional network visualization tool NetGestalt (\href{http://www.netgestalt.org}{http://www.netgestalt.org}). Meanwhile, the NetSAM function also outputs a list object in R which contains all information in "nsm" file.

When \emph{NetGestalt} is FALSE, the NetSAM function outputs three files to make users easily analyze the network. 
1. "*.gmt" file contains all modules found by the NetSAM function. The first column is the hierarchical label which implies the module position in the hierarchical tree. For example, "01-04" means the module is the fourth child of the parent module "01" (root module). The second columns is the number of genes contains in the module. And other columns are the genes in the module. All columns are separated by tab.
2. "*.rul" file contains all genes in the network which are reordered by OLO method.
3. "*.net" file contains the network under analysis which is a simple network by removing the self interactions and loop interactions from the input network.
The NetSAM function also outputs a list object in R which contains all information in three files.
